...
!function (t, i) {
    ...
    return l.find("a").attr("href", "javascript:void(0);"), s.content = l, s.title = l.find(".title"), s.choseAll = l.find(".chose-all"), s.choseMinute = l.find(".chose-minute"), s.choseHour = l.find(".chose-hour"), s.hourShow = l.find(".js-hour-show"), s.minuteShow = l.find(".js-minute-show"), s.update = function () {
        ...
        t.hasBind || (t.hasBind = !0, this.content.on("click", ".js-minus-minute", function () {
            ...
            t.minute = +this.getAttribute("data-val"), t.update(), t.choseMinute.hide(), t.choseAll.show(), t.title.text("時間を選択")
        }).on("click", ".js-hour-cell", function () {
            t.hour = +this.getAttribute("data-val"), t.update(), t.choseHour.hide(), t.choseAll.show(), t.title.text("時間を選択")
        }).on("click", function (t) {
            t.stopPropagation()
        }), t.hourShow.on("click", function () {
            t.choseAll.hide(), t.choseHour.show(), t.title.text("時間を選択")
        }), t.minuteShow.on("click", function () {
            t.choseAll.hide(), t.choseMinute.show(), t.title.text("時間を選択")
        }))
        ...
        if (this[0].nodeName && "INPUT" === this[0].nodeName) return this.$timepickerUpdate = n, this.off("click").on("click", function (i) {
            ...
            if ($(this).closest('table').length > 0) {
                var pos = $(this).offset();
                var table = $('#table_list_schedule').offset()
                console.log(screen.width)
                var temp = 0;
                if(screen.width < 690) {
                    temp = 60;
                }
                var h = this.offsetLeft + 120 + "px", a = pos.top + this.offsetHeight - table.top + 90 +temp+ "px";
                // console.log("> top = "+ a);
