let STUDENT_REGISTER = {};
let routeRegisterStudentValidation = $("[name=route-register-student-validation]").attr('content');
let inputErrorCommon = $("[name=input-error-common]").attr('content');
let inputErrorCheckRequired = $("[name=input-error-check-required]").attr('content');
let routeSendMailToUpdateAuth = $("[name=route-send-mail-to-update-auth]").attr('content');
let routeRegisterStudentSave = $("[name=route-register-student-save]").attr('content');
let token = $("[name=csrf-token]").attr('content');
let imageSize = $("[name=image-size]").attr('content');
let imageFormat = $("[name=image-format]").attr('content');
let inputErrorRadioRequired = $("[name=input-error-radio-required]").attr('content');
let routeRegisterStudentPayment = $("[name=route-register-student-payment]").attr('content');
let routeValidationStudentPayment = $("[name=route-validation-student-payment]").attr('content');
    ...
    STUDENT_REGISTER.handleChoiceImage = function() {
        ...
        $("#image_photo").change(function () {
            if (this.files && this.files[0]) {
                let pic_size = $('#image_photo')[0].files[0].size/1024/1024;//get file size (MB)
    ...
    */
    STUDENT_REGISTER.validationRegisterStudent = function() {
        $('#btnGoToStep2').click(function(e) {
            ...
            $('#day').removeClass('error-brithday');
            // Ajax validation
            $.ajaxSetup({
                headers: {
            ...
            e.preventDefault();
            let data = STUDENT_REGISTER.getDataForm();
            $("#loading").show();
            $.ajax({
                type: "POST",
